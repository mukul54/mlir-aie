...
def my_matmul(
    ...
    def device_body():
        ...
        B_l2_ty = np.ndarray[(k * nb,), np.dtype[dtype_in]]  # packed int4 pairs
        C_l2_ty = np.ndarray[(m * n * n_aie_rows,), np.dtype[dtype_out]]
        A_l1_ty = np.ndarray[(m, k), np.dtype[dtype_in]]
        B_l1_ty = np.ndarray[(k, nb), np.dtype[dtype_in]]  # packed int4 pairs
        ...
        for row in range(n_aie_rows):
            for col in range(n_aie_cols):
                ...
                @core(core_tiles[row][col], f"mm_w4a8_{m}x{k}x{n}.o", stack_size=0xD00)
                def core_body():
        ...
        @runtime_sequence(
            ...
            np.ndarray[(K * Nb,), np.dtype[dtype_in]],  # B packed
                ...
                for pingpong in [0, 1]:
                    ...
                    for col in range(n_aie_cols):
                        ...
                        for tile_row in range(tb_n_rows):
                            ...
                            B_col_offset = col * nb
                            if not b_col_maj:
                                B_sizes = [N // n // n_aie_cols, K // k, k, nb]
                                B_strides = [nb * n_aie_cols, k * Nb, Nb, 1]
                            else:
