...
def my_matmul(
    ...
    # svdq v2: mixed contract (int8 codes in, f32 out) is intentional.
    assert (
    ...
    # --- W4A8 (svdq_gemm v1): B is int4-PACKED along N pairs into bytes.
    # Host B layout: (K, N//2) int8, byte(r,cc) = (B[r,2cc] & 0xF) | (B[r,2cc+1] << 4).
    # All B-side extents use nb = n//2 bytes; unpack happens in-core before the
    # 8x8x8 int8 MMUL. A and C are unchanged int8.
    assert not b_col_maj, "w4a8 design supports row-major B only"
    assert n % 2 == 0 and t % 2 == 0 and N % 2 == 0
    nb = n // 2
    tb4 = t // 2
    Nb = N // 2
    assert nb % tb4 == 0
    # svdq v2: each B tile carries n bf16 per-column scales after the nibble
    # blocks (TileFuse-style inline metadata). Streamed LINEARLY (host
    # pre-tiled into kernel walk order) -> no dims_to_stream, no dma_bd limits.
    btile = k * nb + 2 * n          # bytes per streamed B tile
    n_jb = 0  # set below once n_aie_cols known (jb tiles per column)
    ...
    def device_body():
        ...
        B_l2_ty = np.ndarray[(btile,), np.dtype[dtype_in]]  # packed blocks + scales
        C_l2_ty = np.ndarray[(m * n * n_aie_rows,), np.dtype[dtype_out]]
        A_l1_ty = np.ndarray[(m, k), np.dtype[dtype_in]]
        B_l1_ty = np.ndarray[(btile,), np.dtype[dtype_in]]  # packed blocks + scales
        ...
        for col in range(n_aie_cols):
            ...
            B_l2l1_fifos[col] = object_fifo(
                ...
                fifo_depth,
                B_l1_ty,
                # linear streaming: dims_to_stream omitted (host pre-tiles B)
            )
        ...
        for row in range(n_aie_rows):
            for col in range(n_aie_cols):
                ...
                @core(core_tiles[row][col], f"mm_w4bf16_{m}x{k}x{n}.o", stack_size=0xD00)
                def core_body():
        ...
        @runtime_sequence(
            ...
            np.ndarray[((N // n) * (K // k) * btile,), np.dtype[dtype_in]],  # B pre-tiled+scales
        ...
        def sequence(A, B, C):
            ...
            # keep every C transfer to one tile row when the row-of-tiles stride would
            # exceed the 1M-word npu_dma_memcpy_nd limit (needed for M=768, N=16384)
                ...
                for pingpong in [0, 1]:
                    ...
                    for col in range(n_aie_cols):
                        ...
                        for tile_row in range(tb_n_rows):
                            ...
                            #      ----------------
                            n_jb = N // n // n_aie_cols
                            B_col_offset = col * n_jb * (K // k) * btile
                            if not b_col_maj:
                                # innermost DMA dim is limited to 1023 32-bit words;
                                # split btile so each inner chunk stays under it.
                                B_split = 1
                                while (btile // B_split) > 4092 or btile % B_split != 0:
                                    B_split += 1
                                B_sizes = [n_jb, K // k, B_split, btile // B_split]
                                B_strides = [(K // k) * btile, btile, btile // B_split, 1]
                            else:
